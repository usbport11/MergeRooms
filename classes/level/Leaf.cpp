...
std::vector<NRectangle2> CreateHalls3(NRectangle2* pRoom1, NRectangle2* pRoom2) {
	...
	std::cout<<"("<<Max.x<<" "<<Min.x<<") ("<<Max.y<<" "<<Min.x<<") | "<<HallSize.x<<" "<<HallSize.y<<std::endl;//rand() % (SizeMin + 1) + 1<<std::endl;
	
	int NeedCase = rand() % 2;
	switch(NeedCase) {
		case 0:
			std::cout<<"Simple"<<std::endl;
			//AddPart = NRectangle2(Min.x, Min.y, Max.x - Min.x + HallSize.x, Max.y - Min.y + HallSize.y);
		...
		case 1:
			...
			if(Min.x == Max.x || Min.y == Max.y) {
				if(Min.x == Max.x) {
					//AddPart = NRectangle2(Min.x, Min.y, HallSize.x, Max.y - Min.y + HallSize.y);
					AddPart = NRectangle2(Min.x, Min.y, (rand() % 2 + 1), Max.y - Min.y + (rand() % 2 + 1));
				}
				if(Min.y == Max.y) {
					//AddPart = NRectangle2(Min.x, Min.y, Max.x - Min.x + HallSize.x, HallSize.y);
			...
			if((Min == Room1Point && Max == Room2Point) || (Min == Room2Point && Max == Room1Point)) {
				...
				switch(rand() % 2) {
					case 0:
						...
						AddPart = NRectangle2(Min.x, Min.y, Max.x - Min.x + (rand() % 2 + 1), (rand() % 2 + 1));
						Halls.push_back(AddPart);
						//AddPart = NRectangle2(Max.x, Min.y, HallSize.x, Max.y - Min.y + HallSize.y);
					...
					case 1:
						...
						AddPart = NRectangle2(Min.x, Min.y, (rand() % 2 + 1), Max.y - Min.y + (rand() % 2 + 1));
						Halls.push_back(AddPart);
						//AddPart = NRectangle2(Min.x, Max.y, Max.x - Min.x + HallSize.x, HallSize.y);
